...
<div {if $PAY24_MODULE_DEBUG neq 1}style="display:none;"{else}style="display:inline-block;background-color: #acacac;padding: 15px;"{/if}>
    <form method="post" action="{$PAY24_ACTION}" id="veintequatro-form">
        ...
        {/if}
        {if $PAY24_NOTIFY_CLIENT eq 1}
            <label>NotifyClient</label><br/>
            <input type="text" name="NotifyClient" value="{$PAY24_EMAIL}" /><br/>
        {/if}
        {if $PAY24_NOTIFY eq 1}
        ...
        {/if}
        {if $PAY24_CART eq 1}
            <input type="hidden" name="Cart" value="{$PAY24_CART_CONTENT}" /><br/>
        {/if}
        <br/>
